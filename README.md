...
- **DDP / torchrun / Accelerate (several processes):** Each rank runs the search on **its** assigned GPU (`cuda_mem_devices` default is that GPU only). When ``WORLD_SIZE > 1``, ``find_max_minibatch`` **by default** takes the **minimum** successful trial size across ranks (JSON file sync) before returning, so every rank agrees on the same limit. Pass ``distributed_sync_dir=…`` to a directory on **shared** storage (or set ``BATCH_FINDER_SYNC_DIR``); otherwise sync files go under ``$WORK/.cache`` if ``WORK`` is set, else ``$HOME/.cache``. Actual paths are ``<base>/find_max_minibatch_sync/<job_tag>/`` (``job_tag`` from ``SLURM_JOB_ID`` or ``MASTER_ADDR``+``MASTER_PORT``, etc.). The rank that finishes first **waits** in ``time.sleep`` loops until slower ranks finish their search — progress lines are printed about every 30s so this does not look like a hang.
