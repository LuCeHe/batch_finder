...
- **DDP / torchrun / Accelerate (several processes):** Each rank runs the search on **its** assigned GPU (`cuda_mem_devices` default is that GPU only). When ``WORLD_SIZE > 1``, ``find_max_minibatch`` takes the **minimum** successful trial size across ranks (JSON sync) before returning. Use ``BATCH_FINDER_SYNC_DIR`` or the default ``$WORK/.cache`` / ``$HOME/.cache`` — **do not** pass a per-process random ``output_dir`` (e.g. each worker re-importing a script with a new random experiment id); every rank must resolve the **same** sync directory. Layout: ``<base>/find_max_minibatch_sync/<job_tag>/``. The first rank done **waits** (poll every 4s, status log about every 30s) until others finish searching.
