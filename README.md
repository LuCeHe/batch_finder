...
- 🔍 **Explicit inputs** – Names, dtypes, and rough shapes from `input_shapes` / `forward_params` and optional `config`
...
# ``forward_params`` must list tensor arguments to ``forward`` in order (after batch_finder’s skips),

# same as ``inspect.signature(get_model().forward)`` would show — adjust for your checkpoint.
...
| `get_model` | `Callable[[], nn.Module]` | (required) | **First argument.** Returns a fresh `nn.Module` per attempt; parent never keeps weights. Use with `input_shapes` **dict** or **`forward_params`**. Picklable under `spawn` (module-level function, not `lambda`) |

| `config` | any | `None` | Optional; if omitted, `get_model()` is called once, `.config` is read, module discarded |

| `forward_params` | `Sequence[str]` | `None` | When `input_shapes` is not a dict: ordered tensor names for `forward` |
...
| `use_subprocess` | `bool` | `None` | Default: **subprocess** on Linux/macOS (worker holds the model). `BATCH_FINDER_SUBPROCESS=0` forces in-process. Windows: always in-process |
...
- **OOM:** CUDA OOM and similar errors are caught and the search continues. On **Linux/macOS**, attempts usually run in a **subprocess** (parent has no weights; worker is killable). Env `BATCH_FINDER_SUBPROCESS=0` forces in-process. After failures, `gc.collect()` and device cache clears run where applicable.
